# Third-Party Licenses

This document contains the licenses of all third-party dependencies used in this project.

| Name               | Version             | License                              |
|--------------------|---------------------|--------------------------------------|
| Django             | 6.0.1               | BSD-3-Clause                         |
| Faker              | 40.1.2              | MIT License                          |
| PyYAML             | 6.0.3               | MIT License                          |
| Pygments           | 2.19.2              | BSD License                          |
| annotated-doc      | 0.0.4               | MIT                                  |
| annotated-types    | 0.7.0               | MIT License                          |
| anyio              | 4.12.1              | MIT                                  |
| asgiref            | 3.11.0              | BSD License                          |
| bazis-test-utils   | 0.0.3rc0.post1.dev2 | Apache Software License              |
| certifi            | 2026.1.4            | Mozilla Public License 2.0 (MPL 2.0) |
| charset-normalizer | 3.4.4               | MIT                                  |
| click              | 8.3.1               | BSD-3-Clause                         |
| factory_boy        | 3.3.3               | MIT License                          |
| fastapi            | 0.128.0             | MIT                                  |
| h11                | 0.16.0              | MIT License                          |
| httpcore           | 1.0.9               | BSD-3-Clause                         |
| httptools          | 0.7.1               | MIT                                  |
| httpx              | 0.27.0              | BSD-3-Clause                         |
| idna               | 3.11                | BSD-3-Clause                         |
| iniconfig          | 2.3.0               | MIT                                  |
| packaging          | 25.0                | Apache Software License; BSD License |
| pluggy             | 1.6.0               | MIT License                          |
| pydantic           | 2.12.5              | MIT                                  |
| pydantic_core      | 2.41.5              | MIT                                  |
| pytest             | 9.0.2               | MIT                                  |
| pytest-django      | 4.11.1              | BSD License                          |
| pytest-mock        | 3.15.1              | MIT License                          |
| python-dotenv      | 1.2.1               | BSD-3-Clause                         |
| requests           | 2.32.5              | Apache Software License              |
| requests-mock      | 1.12.1              | Apache Software License              |
| requests-toolbelt  | 1.0.0               | Apache Software License              |
| sniffio            | 1.3.1               | Apache Software License; MIT License |
| sqlparse           | 0.5.5               | BSD License                          |
| starlette          | 0.50.0              | BSD-3-Clause                         |
| typing-inspection  | 0.4.2               | MIT                                  |
| typing_extensions  | 4.15.0              | PSF-2.0                              |
| urllib3            | 2.6.3               | MIT                                  |
| uvicorn            | 0.40.0              | BSD-3-Clause                         |
| uvloop             | 0.22.1              | Apache Software License; MIT License |
| watchfiles         | 1.1.1               | MIT License                          |
| websockets         | 16.0                | BSD-3-Clause                         |
